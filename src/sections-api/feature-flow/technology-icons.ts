import type { FeatureFlowTechnology } from '../../components/section/feature-flow/types';

// ----------------------------------------------------------------------

/**
 * Technology → icon map for the `feature-flow` demo data factories only
 * (issue #171). This is exactly the set of technology names referenced
 * across the six canonical items in `data.tsx` — not a general-purpose icon
 * registry, and not exported from the package barrel. `FeatureFlowTechnology`
 * itself has no opinion on icon resolution (see its own JSDoc): each
 * consumer owns that mapping, and this is simply this data layer's own.
 *
