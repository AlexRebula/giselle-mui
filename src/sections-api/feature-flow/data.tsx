...
import { QuoteCard } from '../../components/material/surfaces/card/quote';
import type {
  FeatureFlowImage,
  FeatureFlowItem,
  FeatureFlowSectionProps,
} from '../../components/section/feature-flow/types';
import { canonicalImagePaths } from './image-paths';
import { toTechnologies } from './technology-icons';

// ----------------------------------------------------------------------
//
// `feature-flow` demo data factories (issue #171) — the pure data layer for
// `feature-flow-section.stories.tsx`. Real content transcribed literally
// from a private consuming app's own production data, minus the fields this
// component intentionally does not support (`outcomes`, `highlights`,
// `ctaLabel`/`ctaHref`, `expertiseSections` override — see the parent
// nesting-policy decision). Not exported from the package barrel: this is a
// data layer for Storybook/demo purposes only. See `src/sections-api/README.md`
// for the pattern this folder follows.
//
// ----------------------------------------------------------------------
...
// ----------------------------------------------------------------------
// Synthetic edge cases — placeholder/synthetic data, deliberately NOT the
// real canonical content, so each edge case stays obviously separate from
// the six canonical items above. Each item below omits exactly one optional
// field to demonstrate the component still renders correctly without it.
// ----------------------------------------------------------------------

/** No `imgUrl` — the hover-stack image column falls back to the shared image config. */
const itemWithNoImage: FeatureFlowItem = {
  id: 'edge-no-image',
...
const itemWithNoTechnologies: FeatureFlowItem = {
...
const itemWithNoMetrics: FeatureFlowItem = {
  id: 'edge-no-metrics',
...
const itemWithNoHighlightCards: FeatureFlowItem = {
...
const itemWithVeryLongDescription: FeatureFlowItem = {
...
// ----------------------------------------------------------------------
// Factories — the data layer's only public surface (re-exported via
// `api.ts`). Shaped so a future swap to a `giselle-sections-sdk`-style
// provider only ever needs to change what happens *inside* these functions:
// every caller already treats this as "ask for the data, get typed objects
// back" rather than importing static consts directly.
// ----------------------------------------------------------------------

/**
 * Section-level config for the canonical story — everything except `items` and `image`.
 * Named via `Pick` (rather than `Omit<FeatureFlowSectionProps, 'items' | 'image'>`) so
 * TypeScript only has to resolve the handful of keys actually used here, not re-map every
 * inherited `BoxProps` key (including `ref`, whose type depends on a generic default that
 * an `Omit` re-computation would otherwise lose).
 */
export function createFeatureFlowCanonicalSectionConfig(): Pick<
  FeatureFlowSectionProps,
  | 'title'
  | 'txtGradient'
  | 'description'
...
/** The sticky image column's source(s) for the canonical story. */
export function createFeatureFlowCanonicalImage(): FeatureFlowImage {
  return {
...
export function createFeatureFlowCanonicalItems(): FeatureFlowItem[] {
  return [
    performanceUi,
    techDebt,
    pragmaticConsulting,
...
/** The five synthetic edge-case items, one per omitted optional field. */
export function createFeatureFlowEdgeCaseItems(): {
  noImage: FeatureFlowItem;
  noTechnologies: FeatureFlowItem;
  noMetrics: FeatureFlowItem;
  noHighlightCards: FeatureFlowItem;
  veryLongDescription: FeatureFlowItem;
} {
  return {
    noImage: itemWithNoImage,
    noTechnologies: itemWithNoTechnologies,
    noMetrics: itemWithNoMetrics,
    noHighlightCards: itemWithNoHighlightCards,
    veryLongDescription: itemWithVeryLongDescription,
  };
}
