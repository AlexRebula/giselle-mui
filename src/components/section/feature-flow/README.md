...
| `itemDetailSx`         | `SxProps<Theme>`                                                  | —                              | Merged onto the expanded detail panel's own root, after `detailPanelSx` — overrides its default tinted background/border-top without needing a full `renderHighlightPanel` override just to change the panel's own chrome.                                   |
